...
class TradeLedger:
    ...
    def build_stats(self, entries: list[dict[str, Any]]) -> dict[str, Any]:
        pnl_usd_values = [_to_float(item.get("pnl_usd")) for item in entries]
        pnl_r_values = [_to_float(item.get("pnl_r")) for item in entries]
        closed_entries = [item for item in entries if item.get("closed_at")]
        wins = [item for item in closed_entries if (_to_float(item.get("pnl_usd")) or 0.0) > 0]
        losses = [item for item in closed_entries if (_to_float(item.get("pnl_usd")) or 0.0) < 0]
        ...
        average_pnl = mean([value for value in pnl_usd_values if value is not None]) if any(value is not None for value in pnl_usd_values) else 0.0
        ...
        total_pnl = sum(value or 0.0 for value in pnl_usd_values)
    ...
    def _sync_markdown(self, entries: list[dict[str, Any]]) -> None:
        ...
        for item in entries:
            lines.append(
                "| {id} | {symbol} | {side} | {setup} | {entry_price} | {exit_price} | {opened_at} | {closed_at} | {pnl_usd} | {pnl_r} | {notes} |".format(
                    **{key: self._escape_markdown_cell(value) for key, value in item.items()}
                )
            )
